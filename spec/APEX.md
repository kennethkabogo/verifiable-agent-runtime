...
**Planned: §14.N — Two-segment session with TEMPORAL_PROOF.** Test vectors
covering the `TEMPORAL_PROOF` packet (`0x09`) and the `TemporalProofHash`
sealed payload field are not yet included. They will be added once the
reference runtime emits conformant `TEMPORAL_PROOF` packets. The recommended
fixture shape is a minimal two-segment session:

```text
SESSION_START[1] → EVIDENCE[2] → TEMPORAL_PROOF[3] → SESSION_RESUME[4] → SETTLEMENT_FINAL[5]
```

with fixed Argon2id params (`m=65536, t=3, p=1`), a seeded SessionID, and
all-zero PCRs (simulation mode §11).

**Argon2id latency (§5.7) is now settled.** The benchmark blocking this fixture
has been completed on the reference hardware: an AWS Nitro Enclave c5.xlarge
(2 vCPU, 512 MiB, production mode, real NSM attestation). Measured latency at
the floor params is **~240 ms** (mean 239.81 ms, p50 241.22 ms, p95 247.09 ms;
n=7). The §5.7 performance note has been updated accordingly. The §14.N fixture
is unblocked and will be generated once `TEMPORAL_PROOF` emission is implemented
in `src/runtime/shell.zig`.
