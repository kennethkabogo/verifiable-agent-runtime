...
all-zero PCRs (simulation mode §11). This fixture SHOULD be generated by
the same benchmark run that measures Argon2id latency inside the target TEE
platform — the wire bytes fall out of the run at no additional cost and the
measured latency replaces the estimate in §5.7.
